...
public class CalculChemins {
    ...
    public static HashMap<List<Position>,Stack> calculerChemins(){
        var map = new HashMap<List<Position>,Stack>();
    ...
    static boolean isValid( int rows, int cols,
                    Position point)
    {
        if (rows > 0 && cols > 0)
    ...
    static boolean isUnBlocked(int[][] grid, int rows, int cols,
                        Position point)
    {
            ...
            Position dest)
    {
        ...
        //System.out.println(path.getFirst()+" 1");
        //celui du milieu
        //System.out.println(path.get(path.size()/2)+" milieu");
    ...
    static Stack aStarSearch(int[][] grid,
                     int rows,
                     int cols,
                     Position src,
                     Position dest)
    {
        ...
        cellDetails[i][j].parent = new Position( i, j );
        ...
        while (!openList.isEmpty()) {
            ...
            for (int addX = -1; addX <= 1; addX++) {
                for (int addY = -1; addY <= 1; addY++) {
                    Position neighbour = new Position(i + addX, j + addY);
                    if (isValid( rows, cols, neighbour)) {
                        if(cellDetails[neighbour.getX()] == null){ cellDetails[neighbour.getX()] = new Cell[cols]; }
                        if (cellDetails[neighbour.getX()][neighbour.getY()] == null) {
                            cellDetails[neighbour.getX()][neighbour.getY()] = new Cell();
                        }

                        if (isDestination(neighbour, dest)) {
                            cellDetails[neighbour.getX()][neighbour.getY()].parent = new Position ( i, j );
                            //System.out.println("The destination cell is found");
                            return tracePath(cellDetails, dest);
                        }

                        else if (!closedList[neighbour.getX()][neighbour.getY()]
                            ...
                            if (cellDetails[neighbour.getX()][neighbour.getY()].f == -1
                                    || cellDetails[neighbour.getX()][neighbour.getY()].f > fNew) {

                                openList.add(new Details(fNew, neighbour.getX(), neighbour.getY()));
                                cellDetails[neighbour.getX()][neighbour.getY()].g = gNew;
