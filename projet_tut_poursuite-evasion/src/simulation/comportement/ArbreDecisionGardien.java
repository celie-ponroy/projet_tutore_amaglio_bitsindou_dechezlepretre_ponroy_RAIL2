    ...
    public class ArbreDecisionGardien extends ArbreDecision implements Comportement  {
        ...
        public Deplacement prendreDecision() {
            ...
            if(simulation.estVisible(personnage, false)){
                ...
                Stack<Position> s =Simulation.CHEMIN.get(List.of(simulation.getPrisonnier().getPosition(),Simulation.getPosSortie()));
                if(s.size()>2){
                    Stack<Position> sMoitie =Simulation.CHEMIN.get(List.of(simulation.getPrisonnier().getPosition(),s.get(s.size()/2)));
                ...
                if(probas.size() == 1){
                    Stack<Position> sProba =Simulation.CHEMIN.get(List.of(simulation.getGardien().getPosition(),new Position(probas.get(0).getX(), probas.get(0).getY())));
                ...
                Stack<Position> s2 = Simulation.CHEMIN.get(List.of(personnage.getPosition(), new Position(c.getX(), c.getY())));
                if (s2.empty()){
